...
const LeadCostCalculator = () => {
  ...
  const toggleSelection = (group, key) => {
    const newGroup = { ...Object.fromEntries(Object.keys(group).map(k => [k, false])), [key]: !group[key] };
    return newGroup;
  };
      ...
      <div className="w-full max-w-6xl bg-white rounded-lg shadow-lg overflow-hidden flex flex-col md:flex-row">
        ...
        <div className="w-full md:w-1/2 p-8">
          ...
          <div className="mb-6">
            ...
            <div className="flex gap-3">
              {['mobile', 'email'].map(type => (
                <button
                  key={type}
                  className={`px-5 py-2 rounded-full border text-sm font-medium transition ${contactGroup[type] ? 'bg-white border-gray-400 shadow-md' : 'bg-white border-gray-300'}`}
          ...
          <div className="mb-6">
            ...
            <div className="flex gap-3">
              {['mumbai', 'thane'].map(loc => (
                <button
                  key={loc}
                  className={`px-5 py-2 rounded-full border text-sm font-medium transition ${locationGroup[loc] ? 'bg-white border-gray-400 shadow-md' : 'bg-white border-gray-300'}`}
          ...
          <div className="mb-6">
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                <button
                  key={b.key}
                  className={`px-5 py-2 rounded-full border text-sm font-medium transition ${budgetGroup[b.key] ? 'bg-white border-gray-400 shadow-md' : 'bg-white border-gray-300'}`}
                  onClick={() => setBudgetGroup(toggleSelection(budgetGroup, b.key))}
                >
                  {b.label}
                </button>
              ))}
          ...
          <div className="mb-6">
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                ...
                <button
                  key={e.key}
                  className={`px-5 py-2 rounded-full border text-sm font-medium transition ${enquiryGroup[e.key] ? 'bg-white border-gray-400 shadow-md' : 'bg-white border-gray-300'}`}
                  onClick={() => setEnquiryGroup(toggleSelection(enquiryGroup, e.key))}
                >
                  {e.label}
                </button>
              ))}
